...
import {resetUpdateProfile, updateUserProfile,} from "../redux/slices/authSlice";
...
const EditProfile = () => {
    ...
    const fetchDataUser = async () => {
        try {
            const userProfile = await dispatch(getUserProfile(username));
            if (userProfile && userProfile.payload) {
              setAvatar(userProfile.payload?.avatar?.url || null)
              setCover(userProfile.payload?.cover?.url || null)
              setFirstname(userProfile.payload?.firstName || "")
              setLastname(userProfile.payload?.lastName || "")
              setEmail(userProfile.payload?.email || "")
              setBio(userProfile.payload?.bio || "")
              setUserName(userProfile.payload?.username || "")
            }
        } catch (error) {
            console.error(error);
        }
                                        ...
                                        <section>
                                            ...
                                            <div className="section-content">
                                                ...
                                                <Form.Group className="form-group mb-3">
                                                    ...
                                                    />
                                                    <Form.Label>zalo URL</Form.Label>
                                                </Form.Group>
                                            </div>
                                        </section>
                                        <section>
